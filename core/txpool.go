...
type Tx_pool struct {
	...
	// Heap TxHeap
	// relay到不同分片的交易队列
	Relay_Pools map[string][]*Transaction2
	//锁
	...
	Outing_Before_Announce_TX_Pools map[string][]*Transaction2

	// 用来收集Outing_Acc的要进入交易池的交易，由Outing_Acc_After_Announce_Lock控制
	Outing_After_Announce_TX_Pools map[string][]*Transaction2

	// 用来收集交易池中锁住账户的交易，由Lock_Acc_Lock控制
	Locking_TX_Pools map[string][]*Transaction2
...
func NewTxPool() *Tx_pool {
	pool := Tx_pool{
		Queue:                           make([]*Transaction2, 0),
		Relay_Pools:                     make(map[string][]*Transaction2),
		Migration_Pool:                  make(map[string]int),
		Outing_Before_Announce_TX_Pools: make(map[string][]*Transaction2),
...
func (pool *Tx_pool) AddTx(tx *Transaction2) {
	pool.Queue = append(pool.Queue, tx)
	// heap.Push(&pool.Heap, tx)
}

func (pool *Tx_pool) AddTxs(txs []*Transaction2) {
	pool.Lock.Lock()
...
func (pool *Tx_pool) MRandPick() (tx *Transaction2) {
	...
	// 翻新交易池，避免出现内存不回收的情况
	new_queue := make([]*Transaction2, 0)
	for i, v := range pool.Queue {
		if i != index {
			new_queue = append(new_queue, v)
		}
	}
...
func (pool *Tx_pool) FetchTxs2Pack(left_count, blockNumber int) (txs []*Transaction2, queueLen int) {
	...
	for k, v := range pool.Queue {
		// MMM v是一个单独的交易，这里是直接遍历pool.Queue
		if tx_cnt == 0 {
			break
		}
		loc = k
