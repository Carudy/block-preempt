...
func (bc *BlockChain) getUpdatedTreeOfState(commit int, height int, txs []*core.Transaction2, mig1s []*core.TXmig1, mig2s []*core.TXmig2, anns []*core.TXann, nss []*core.TXns) ([]byte, map[string]*big.Int) {
	...
	for _, tx := range txs {
		...
		for i := 0; i < mlen; i ++{
			...
			if r_state_enc == nil {
				fmt.Printf("rec属于该分片吗：%v\n", account.AccountInOwnShard[hex.EncodeToString(tx.Recipient[i])])
				fmt.Printf("rec地址为：%v\n", hex.EncodeToString(tx.Recipient[i]))
				fmt.Printf("rec属于分片：%v\n", account.Account2Shard[hex.EncodeToString(tx.Recipient[i])])
				fmt.Printf("txid：%v\n", tx.Id)
				log.Panic()
			...
			st.Update(tx.Recipient[i], account_state.Encode())
		}
	}
...
func GetTxTreeRoot(txs []*core.Transaction2) []byte {
