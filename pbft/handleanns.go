...
func (p *Pbft) handleAnns(anns []*core.TXann, st *trie.Trie, migTree *trie.Trie) (int64, int64) {
	...
	utils.Mzlog(fmt.Sprintf("handleAnns时是第%v个块\n", p.epoch-1))
	...
	for _, ann := range anns {
		...
		// MMM 输出要迁移的账户地址
		//utils.Mzlog(fmt.Sprintf("  > ann.Address: %v", ann.Address))
		...
		// MMMM 这个结构体比较重要，是用来管理迁移pending交易的
		...
		if params.Config.Lock_Acc_When_Migrating {
			...
			for _, tx := range p.Node.CurChain.Tx_pool.Locking_TX_Pools[ann.Address] {
				...
				for i := 0; i < mlen; i ++{
					...
					if hex.EncodeToString(tx.Recipient[i]) == ann.Address && !tx.IsRelay && !tx.Relay_Lock{
	...
	for _, tx := range p.Node.CurChain.Tx_pool.Queue {
		...
		for i := 0; i < mlen; i ++{
			...
			//  如果recipient是要出去的账户，且已经是relaytx，则收集
			if shard, ok := isdelete[hex.EncodeToString(tx.Recipient[i])]; ok && (tx.IsRelay || tx.Relay_Lock) {
				// 在半锁机制中，即使这些交易没有被锁，也通过这样做个标记，表明是被迁移过去的
				tx.RecLock = true
				cAps[shard][hex.EncodeToString(tx.Recipient[i])].PendingTxs = append(cAps[shard][hex.EncodeToString(tx.Recipient[i])].PendingTxs, tx)
				// continue
				need_continue = true
				break
			}
	...
	utils.Mzlog(fmt.Sprintf("> cAps len: [%v] ", len(cAps)))
	// MMM 输出测试
	// for _, caps := range cAps {
	// 	for _, cAp := range caps {
	// 		for _i, tx := range cAp.PendingTxs{
	// 			utils.Mzlog(fmt.Sprintf("  > [%v] tx from [%v] to [%v] value: [%v]", _i, hex.EncodeToString(tx.Sender), hex.EncodeToString(tx.Recipient), tx.Value))
	// 		}
	// 	}
	// }
...
func (p *Pbft) mpAndSend(isdelete map[string]string, anns []*core.TXann, cAps map[string]map[string]*ChangeAndPending, st, migTree *trie.Trie) {
	...
	for _, ann := range anns {
		...
		state := account.DecodeAccountState(encoded)   // MMM 从tree获取账户状态
