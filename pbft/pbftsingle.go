...
func (p *Pbft) commit1(content []byte, pbftType string) {
	// MMMM 如果是区块 目前的情况是pbftType只有Block


	utils.Mzlog(fmt.Sprintf("call commit1 !!! [%v]", pbftType))

	if pbftType == "Block" {
		block := core.DecodeBlock(content)   // MMM 解码区块
		...
		if p.Node.nodeID == "N0" {
			...
			real_tx_total := 0    // MMM 计算真实交易数量
			for _, tx := range block.Transactions{
				real_tx_total += len(tx.Recipient)
			}
			...
			for _, v := range block.Transactions {
				...
				need_break := false  // MMM 需要跳出mlen循环
				add_relay := false // MMM 添加过
				add_relay_count := false  // MMM
				mlen := len(v.Recipient)
				for i := 0; i < mlen && need_break == false; i ++{
					...
					// txlog.Write(strings.Split(s, " "))
					if v.IsRelay && add_relay_count == false {
						relayCount++
						add_relay_count = true
					} else if toid != params.ShardTable[params.Config.ShardID] {
						v.IsRelay = true
						if add_relay == false{
							relaytxs = append(relaytxs, v)
							add_relay = true  // 防止添加第二次
						}
						if add_relay_count == false{
							relayCount++
							add_relay_count = true
						}
					}
			...
			utils.Mzlog("pbftsingle.go call handleAnns")
			...
			// 	pbftend-InitTime*1000, 
			// 	block.Header.Number, 
			// 	tx_total, 
			// 	tx_total-relayCount, 
			// 	tx_total-len(commit_txs), 
			// 	relayCount-tx_total+len(commit_txs), 
			// 	len(commit_txs), 
			// 	pbftend-pbftbefore, 
			// 	len(block.TXmig1s), 
			// 	len(block.TXmig2s), 
			...
			// MMMcommit_txs数量重新计算
			commit_txs_total := 0    // MMM 计算真实交易数量
			for _, tx := range commit_txs{
				commit_txs_total += len(tx.Recipient)
			}
			...
			s = fmt.Sprintf("%v %v %v %v %v %v %v %v %v %v %v %v %v %v", 
				...
				block.Header.Number, 
				tx_total, 
				real_tx_total - relayCount, 
				real_tx_total - commit_txs_total,//real_tx_total-len(commit_txs), 
				relayCount - real_tx_total + commit_txs_total, 
				commit_txs_total, //len(commit_txs), 
				pbftend-pbftbefore, 
				len(block.TXmig1s), 
				len(block.TXmig2s), 
				len(block.Anns), 
