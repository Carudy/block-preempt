...
func NewPBFT() *Pbft {
	...
	if config.Stop_When_Migrating {
		...
		intx_pool = make([]*core.Transaction2, 0)
...
// 处理提交确认消息
// MMM 这里对log进行了写入 [没有实际调用]

func (p *Pbft) handleCommit(content []byte) {
	//使用json解析出Commit结构体
	utils.Mzlog("call handleCommit")
		...
		if pbftType == "Block" && !ok {
			...
			if pbftType == "Block" {
				...
				if p.Node.nodeID == "N0" {
					...
					relayCount := 0
					//已上链交易集
					commit_txs := []*core.Transaction2{}
					//要发送relay交易集合
					relaytxs := make([]*core.Transaction2, 0)

					// MMM v->单独一笔交易 v.Recipient->目标地址
					// MMM toid -> 转账目标
					...
					for _, v := range block.Transactions {
						...
						for i := 0; i < mlen; i ++{
							...
							if params.Config.Lock_Acc_When_Migrating {
								...
								// MMM 判断 转账目标 是否处于当前节点的分片
								if params.ShardTable[params.Config.ShardID] == toid {
									// MMM 处于当前分片
									// MMM 判断这个账户有没有被锁
									if account.Lock_Acc[hex.EncodeToString(v.Recipient[i])] {
										v.IsRelay = true
										// if v.LockTime > 0 {
										// 	v.LockTime2 = pbftend
										// } else {
										...
										var s string
										s = fmt.Sprintf("%v %v %v %v %v %v %v %v %v %v %v %v %v %v %v %v %v %v", hex.EncodeToString(v.Sender), hex.EncodeToString(v.Recipient[i]), v.Id, block.Header.Number, v.RequestTime-InitTime*1000, v.Second_RequestTime-InitTime*1000, v.CommitTime-v.RequestTime, v.LockTime-InitTime*1000, v.UnlockTime-InitTime*1000, v.LockTime2-InitTime*1000, v.UnlockTime2-InitTime*1000, v.Success, v.SenLock, v.RecLock, v.HalfLock, v.Sen_Suppose_on_chain, v.Rec_Suppose_on_chain, v.Relay_Lock)
					...
					if params.Config.Lock_Acc_When_Migrating {
						...
						for _, v := range block.TXmig1s {
							...
							p.Node.CurChain.Tx_pool.Locking_TX_Pools[v.Address] = make([]*core.Transaction2, 0)
						}
						account.Lock_Acc_Lock.Unlock()
					}
					...
					if !params.Config.Stop_When_Migrating && !params.Config.Lock_Acc_When_Migrating {
						...
						for _, v := range block.TXmig1s {
							...
							p.Node.CurChain.Tx_pool.Outing_Before_Announce_TX_Pools[v.Address] = make([]*core.Transaction2, 0)
						}
					...
					//处理要被彻底迁移出去的账户
					utils.Mzlog(fmt.Sprintf("call handleAnns from %v", params.Config.ShardID))
					...
					txmig2log.Flush()
					// ctxlog.Flush()
					s = fmt.Sprintf("%v %v %v %v %v %v %v %v %v %v %v %v %v %v", pbftend-InitTime*1000, block.Header.Number, tx_total, tx_total-relayCount, tx_total-len(commit_txs), relayCount-tx_total+len(commit_txs), len(commit_txs), pbftend-pbftbefore, len(block.TXmig1s), len(block.TXmig2s), len(block.Anns), len(block.NSs), locked_cnt, 0)
...
// relay
func (p *Pbft) TryRelay(relaytxs, alltxs []*core.Transaction2, st *trie.Trie, num int) {
	fmt.Printf("\n第%v个块开始准备relay\n\n", num)
	// use a memory trie database to do this, instead of disk database
	triedb := trie.NewDatabase(rawdb.NewMemoryDatabase())
	...
	for _, v := range relaytxs {
		...
		for i:=0; i < mlen; i ++{
			...
			err2 := st.Prove(v.Recipient[i], 0, proofDB2)
			if err2 != nil {
				log.Panic(err2)
			}
			relaypool[shardID] = append(relaypool[shardID], &core.TXrelay{Txcs: v, MPcs: proofDB1, State: account.DecodeAccountState(st.Get(v.Recipient[i])), MPstate: proofDB2})
		}
	}
...
func (p *Pbft) TrySendTX(txs []*core.Transaction2) {
	config := params.Config

	sendpool := make([][]*core.Transaction2, config.Shard_num)
	for _, v := range txs {
	...
	for _, tx := range relay.Txs {
		...
		for i := 0; i < mlen; i ++{
			...
			if !params.Config.Not_Lock_immediately && account.Lock_Acc[hex.EncodeToString(txcs.Recipient[i])] {
				...
				p.Node.CurChain.Tx_pool.Locking_TX_Pools[hex.EncodeToString(txcs.Recipient[i])] = append(p.Node.CurChain.Tx_pool.Locking_TX_Pools[hex.EncodeToString(txcs.Recipient[i])], txcs)
				account.Lock_Acc_Lock.Unlock()
				// continue // MMM 应该改为break
				break
			}
			account.Lock_Acc_Lock.Unlock()
...
// 			p.Node.CurChain.Tx_pool.Outing_After_Announce_TX_Pools[v] = make([]*core.Transaction2, 0)
// 			account.Outing_Acc_After_Announce_Lock.Unlock()

// 			cAp := &ChangeAndPending{}
// 			cAp.PendingTxs = make([]*core.Transaction2, 0)
...
func (p *Pbft) handleAnnounce(content []byte) {
	...
	for _, v := range announce.TXanns {
		...
		if account.AccountInOwnShard[v.Address] {
			...
			p.Node.CurChain.Tx_pool.Outing_After_Announce_TX_Pools[v.Address] = make([]*core.Transaction2, 0)
			account.Outing_Acc_After_Announce_Lock.Unlock()
...
func (p *Pbft) TrySendChangesAndPendings(nss []*core.TXns, caps map[string]*ChangeAndPending, target_shardID string) {
	...
	*/
	...
	if Joint{
		...
		utils.Mzlog("----聚合完成---")
		// utils.Mzlog(fmt.Sprintf("json长度: %v ", len(string(mtest))))
		// utils.Mzlog("-------")
	}
	// MMM 聚合处理完成
	...
	// utils.Mzlog(fmt.Sprintf("TrySendChangesAndPendings > json: %v \n", string(bc)))
...
func (p *Pbft) handleMigrateWanted() {
	...
	utils.Mzlog("call handleMigrateWanted")
