...
func (p *Pbft) SendOut(out map[string]int) {
		...
		for i := 0; i < mlen; i ++{
			...
			if shard, ok := out[hex.EncodeToString(tx.Sender)]; ok && !tx.IsRelay {
				...
				if _, ok2 := outpool[shard].List[hex.EncodeToString(tx.Sender)]; !ok2 {
					...
					outpool[shard].List[hex.EncodeToString(tx.Sender)] = &BalanceAndPending{Balance: balance, PendingTxs: []*core.Transaction2{}}
				}
			...
			if shard, ok := out[hex.EncodeToString(tx.Recipient[i])]; ok && tx.IsRelay {
				if outpool[shard] == nil {
					outpool[shard] = &BalancesAndPendings{}
					outpool[shard].List = make(map[string]*BalanceAndPending)
				}
				if _, ok2 := outpool[shard].List[hex.EncodeToString(tx.Recipient[i])]; !ok2 {
					encoded_state := st.Get(tx.Recipient[i])
					if encoded_state == nil {
						log.Panic()
					}
					state := account.DecodeAccountState(encoded_state)
					balance := state.Balance
					outpool[shard].List[hex.EncodeToString(tx.Recipient[i])] = &BalanceAndPending{Balance: balance, PendingTxs: []*core.Transaction2{}}
				}
				outpool[shard].List[hex.EncodeToString(tx.Recipient[i])].PendingTxs = append(outpool[shard].List[hex.EncodeToString(tx.Recipient[i])].PendingTxs, tx)
				continue
			}
		}
	...
	for addr, shard := range out {
		...
		if _, ok := outpool[shard].List[addr]; !ok {
			outpool[shard].List[addr] = &BalanceAndPending{Balance: big.NewInt(0), PendingTxs: []*core.Transaction2{}}
		}
	}
