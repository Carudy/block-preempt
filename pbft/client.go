...
var (
	...
	commit_tx_set                         []*core.Transaction2
	commit_tx_set_lock                    sync.Mutex
	pending_tx_set                        []*core.Transaction2
...
func Get_Initial_Map_And_TXS(path string, Account2Shard map[string]int) []*core.Transaction2 {
	txs := make([]*core.Transaction2, 0)
	txid := 0
	...
	for i := 0; i < 1000000; i++ {
		...
		// 	log.Panic()
		// }
		if path == "0to999999_BlockTransaction2.csv" || path == "300W.csv" || path == "100W.csv" || path == "50W.csv" || path == "20W.csv" || path == "200W.csv" {
...
func InjectTXS(txs []*core.Transaction2) {
	cnt := 0
	for {
		...
		sendtxlock.Lock()
		account.Account2ShardLock.Lock()
		to_send := make([][]*core.Transaction2, params.Config.Shard_num)
